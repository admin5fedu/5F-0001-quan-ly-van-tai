...
export interface BadgeConfigItem {
  ...
  /** Override palette — dùng cho badge trạng thái duyệt bám template */
  className?: string;
}
...
const EnumBadge: React.FC<EnumBadgeProps> = ({
  ...
  const colorClasses = item?.className ?? (item ? COLOR_CLASSES[item.color] ?? COLOR_CLASSES.slate : COLOR_CLASSES.slate);
