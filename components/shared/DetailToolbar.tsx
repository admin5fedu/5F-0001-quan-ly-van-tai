        ...
        <div className={cn("grid gap-3 p-3.5 min-w-0", gridColsClass, className)}>
            {actions.map((action, index) => {
                ...
                const componentProps = isLink 
                    ? { href: action.href, target: action.target ?? '_blank', rel: 'noopener noreferrer' } 
                ...
                return (
                    <Component
                        ...
                        className={cn(
                            "flex flex-col items-center gap-1.5 transition-[transform,colors] duration-150 outline-none min-w-0 w-full cursor-pointer",
                            action.disabled ? "opacity-40 cursor-not-allowed pointer-events-none" : "hover:-translate-y-0.5 active:scale-95"
                        )}
                    >
                        <div className={cn(
                            ...
                            {React.cloneElement(action.icon as React.ReactElement<any>, { size: 16, strokeWidth: 2 })}
                        </div>
                        <span className={cn(
