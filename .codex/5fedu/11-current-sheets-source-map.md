...
- `vt_chuyen_xe`: `id`, `ngay`, `id_tai_xe`, số chuyến, tổng tiền lương, tổng phí, ghi chú, **`trang_thai` = trạng thái duyệt** `Chưa duyệt/Đã duyệt/Không duyệt`; cột tiến độ `2/4` = CT đã thực hiện/tổng CT (owner 2026-06-15).
- `vt_chuyen_xe_ct`: `id`, `id_chuyen_xe`, `id_dia_diem`, tiền lương, chi phí (tài xế nhập khi báo cáo), ghi chú; **`trang_thai` = thực hiện** (`Chưa thực hiện`/`Đang thực hiện`/`Đã thực hiện`/`Hủy`); **`phe_duyet` = duyệt** (`Chưa duyệt/Đã duyệt/Không duyệt`, cascade từ cha khi duyệt). Hai cột độc lập; CT có thể đã thực hiện nhưng chưa duyệt.
