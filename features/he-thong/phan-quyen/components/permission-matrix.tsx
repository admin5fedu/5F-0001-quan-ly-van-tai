...
const getFirstModuleId = (): string =>
  PERMISSION_FUNCTIONS[0]?.groups?.[0]?.modules?.[0]?.id ?? 'nhan-vien';
...
const MobileModuleDetail: React.FC<{
  ...
  const actionLabels: Record<string, string> = {
    ...
    check: txt('permission.form.check'),
  ...
  const departmentOptions = useMemo(() => {
    ...
    const otherDeptLabel = txt('permission.matrix.otherDept');
    const unique = Object.keys(deptOrder).sort((a, b) => {
      if (a === otherDeptLabel) return 1;
      if (b === otherDeptLabel) return -1;
      return (deptOrder[a] ?? 9999) - (deptOrder[b] ?? 9999);
    });
