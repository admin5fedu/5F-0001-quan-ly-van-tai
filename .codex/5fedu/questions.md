...
## Out of scope dự án TAH (owner 2026-06-14)

Không implement trừ khi owner đổi scope:

- Cloudinary upload/media thật
- Google Sheets/AppSheet kết nối trực tiếp
- Vercel Edge Function / tối ưu egress cuối dự án
...
## Chuyến xe — thực hiện vs duyệt (owner 2026-06-15)

Đã chốt: hai trường riêng; duyệt cha→con; báo cáo từng CT; lương cần **cả** duyệt + đã thực hiện. Spec: `13-trip-execution-vs-approval-spec.md`.

Còn một điểm mở:

- Flow tài xế có cần hiển thị/chọn trạng thái `Đang thực hiện` hay chỉ `Chưa thực hiện` → `Đã thực hiện` / `Hủy`? (Schema/template gốc có đủ 4 giá trị.)
