...
import { cn, getErrorMessage, saveBlobAs } from '../../lib/utils';
...
const ImportDialog: React.FC<ImportDialogProps> = ({
  ...
  const downloadTemplate = async () => {
    ...
    saveBlobAs(new Blob([wbout], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }), `${templateFileName}.xlsx`);
  };
                              ...
                              <tr key={ri} className="border-b border-border/30">
                                {sheetHeaders.map((_, ci) => (
                                  <td key={ci} className="px-2 py-1.5 text-foreground whitespace-nowrap max-w-[150px] truncate">
                                    {row[ci] != null ? String(row[ci]) : <span className="text-muted-foreground/40">--</span>}
                                  </td>
                                ))}
                              </tr>
                            ))}
