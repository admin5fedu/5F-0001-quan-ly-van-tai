...
- **Phân tách Trạng thái Phê duyệt ở cha và Trạng thái Thực hiện ở con**: Cấu hình trạng thái bảng cha (`vt_chuyen_xe`) bắt buộc dùng `APPROVAL_OPTIONS` (`Chưa duyệt`, `Đã duyệt`, `Không duyệt`) để làm căn cứ khóa dòng và chốt sổ dữ liệu. Bảng con (`vt_chuyen_xe_ct`) dùng `TRIP_CHILD_STATUS_OPTIONS` (`Chưa thực hiện`, `Đang thực hiện`, `Đã thực hiện`, `Không thực hiện`) để phản ánh tiến độ thực tế, không dùng chung bộ trạng thái.
- **Popup Phê duyệt phải có Form nhập ý kiến**: Nút phê duyệt (Duyệt/Không duyệt) của các chứng từ/bảng lương/chuyến xe không được xác nhận đơn giản, mà phải mở Popup chứa Form gồm: lựa chọn kết quả (Đã duyệt / Không duyệt) và ô nhập Ý kiến phê duyệt (`ghi_chu`).
