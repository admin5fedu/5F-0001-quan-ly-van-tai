...
4. **Mật khẩu mặc định — QUY TẮC CỨNG (Credentials Convention)**:
 ...
 6. **Popup Xác nhận Bắt buộc cho Mọi Hành động Bảng Con VÀ Nested Detail Drawer (Action Confirmation — QUY TẮC CỨNG)**:
   - Mọi nút hành động trên bảng con nhúng (EmbeddedChildDataGrid) và trên nested detail drawer (như Sửa, Xóa, Quản lý duyệt) đều bắt buộc phải hiển thị hộp thoại popup xác nhận (`confirm()`) trước khi thực hiện.
   ...
   - **Bài học**: Nút sửa dòng con và nút quản lý duyệt dòng con trong drawer từng bị bỏ sót không có popup xác nhận.
