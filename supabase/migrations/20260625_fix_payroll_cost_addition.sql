...
begin
  ...
  values (
    ...
    v_tong_chi_phi,
    0,
    v_luong_co_ban + v_tong_luong + v_tong_chi_phi, -- Sửa thành dấu +
    'Chưa duyệt',
    now()
  )
  on conflict (nam, thang, id_tai_xe) do update set
    ...
    tong_con_lai = excluded.luong_co_ban + excluded.tong_luong_chuyen + excluded.tong_chi_phi_chuyen - coalesce(public.vt_luong.tru_tien_khac, 0), -- Sửa thành dấu +
    tg_cap_nhat = now();
end;
$$;

-- Tính toán lại toàn bộ bảng vt_luong hiện tại theo công thức mới
do $$
declare
  r record;
begin
